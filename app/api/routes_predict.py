# ▪ : Defines the /predict route for price predictions
...
# End-point for making ppredictions
@router.post('/predict')
def predict_price(car: CarFeatures, user=Depends(get_current_user), _=Depends(get_api_key)):
    prediction = predict_car_price(car.model_dump()) # Converts fastapi json output into python dictionary
